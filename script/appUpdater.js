(function () {
  ...
  function scoreApkAsset(asset, normalizedTagVersion) {
    ...
    score += Math.floor(size / 1024 / 1024);
    return score;
  }
  ...
  async function resolveCurrentVersion(config) {
    ...
    try {
      ...
      if (info && typeof info.version === "string" && info.version.trim()) {
        return { version: info.version, usedFallback: false };
      }
    } catch (error) {
      console.warn("[updater] Failed to read runtime app version, using fallback.", error);
    }

    return { version: fallback, usedFallback: true };
  }
  ...
  function buildPromptMessage(updateInfo, currentVersion, platform) {
    ...
    if (platform === "web") {
      lines.push("You are on the website. This will open the Android APK download.");
      lines.push("");
    }
  ...
  async function checkForUpdates(options) {
    ...
    try {
      ...
      if (!latest.version) throw new Error("Latest version was missing from update source.");
      ...
      if (!downloadUrl) throw new Error("Update source does not provide an APK or release page URL.");

      const shouldOpen = window.confirm(buildPromptMessage(latest, installedVersion, platform));
      if (!shouldOpen) return { status: "cancelled", latest };

      await openDownloadUrl(downloadUrl);

      if (!silent) {
        if (platform === "web") {
          alert("Download page opened in a new tab.");
        } else {
